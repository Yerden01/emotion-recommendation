...
from spotify_recommendation import add_new_songs
...
while True:
    ...
    if len(results) == 1:  # len(results)==1 if there is a face detected. len ==0 if no face is detected
        try:
            ...
            last_expressions.append(maxindex)
            cv2.putText(frame, emotion_dict[maxindex], (x1 + 20, y1 - 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255),
                        2, cv2.LINE_AA)
        except:
            pass
    ...
    if len(last_expressions) > 10:
        print("Last expressions: ", last_expressions)
        add_new_songs(last_expressions)
        last_expressions = []
    # time.sleep(1)
